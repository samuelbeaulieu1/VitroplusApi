...
func (clock *Clock) UpdateEmployeeClocks(req *classes.UpdateEmployeeClocksRequest, clocks *[]models.ClockModel) error {
	...
	for _, c := range *clocks {
		clockDate := c.Date.In(timezone.Local)
		date := time.Date(year, month, day, clockDate.Hour(), clockDate.Minute(), 0, 0, timezone.Local)
...
func (clock *Clock) GetEmployeeClocksBetween(employeeID string, startDate time.Time, endDate time.Time) (*[]models.ClockModel, error) {
	startDate = time.Date(startDate.Year(), startDate.Month(), startDate.Day(), 0, 0, 0, 0, timezone.Local)
	endDate = time.Date(endDate.Year(), endDate.Month(), endDate.Day()+1, 0, 0, 0, -1, timezone.Local)
...
func (clock *Clock) GetEmployeeClocks(employeeID string, date time.Time) (*[]models.ClockModel, error) {
	...
	startDate := time.Date(year, month, day, 0, 0, 0, 0, timezone.Local)
	endDate := time.Date(year, month, day+1, 0, 0, 0, -1, timezone.Local)
